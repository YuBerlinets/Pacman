...
public class PacmanBoardGenerator {
    ...
    private static final int[][][] TEMPLATES = {BOX_2X2, LETTER_T_TOP_3X3, LETTER_T_RIGHT_3X3, BOX_3X3};
    ...
    public PacmanBoardGenerator(int heightInput, int widthInput) {
        ...
        random = new Random();
        int loops = (width * height)/ (width+height)/2;
    ...
    private static void addTemplate(int[][] board, int[][] template, int centerRow, int centerCol) {
        ...
        for (int row = 0; row < templateHeight; row++) {
            for (int col = 0; col < templateWidth; col++) {
                if (startRow + row >= 0 && startRow + row < boardHeight &&
                        startCol + col >= 0 && startCol + col < boardWidth) {
                    board[startRow + row][startCol + col] = template[row][col];
                }
            }
        }
    }
